...
export function RegistrationCardViewer({ santri, appSettings, onClose }: RegistrationCardViewerProps) {
  const handlePrint = () => {
    window.print();
  };

  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[100] p-4 overflow-y-auto no-print">
      <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full overflow-hidden animate-in fade-in zoom-in duration-300">
        <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={handlePrint}
              className="flex items-center gap-2 px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-bold text-sm transition-all shadow-lg shadow-emerald-200 cursor-pointer"
            >
              <PrinterIcon size={18} />
              Cetak Kartu
            </button>
        ...
        <div className="p-8">
          {/* Card to be printed */}
          <div id="registration-card" className="border-4 border-emerald-950 p-8 rounded-xl bg-white relative overflow-hidden print:border-2">
            ...
            <div className="relative z-10">
              ...
              <div className="flex items-center gap-4 border-b-2 border-emerald-950 pb-6 mb-6">
                <div className={`w-16 h-16 rounded-xl flex items-center justify-center font-bold text-2xl overflow-hidden ${appSettings.logoUrl ? 'bg-white' : 'bg-emerald-950 text-white'}`}>
                  {appSettings.logoUrl ? (
                ...
                <div>
                  <h1 className="text-2xl font-black text-emerald-950 uppercase tracking-tight">{appSettings?.pondokName || 'Pondok Pesantren Wahyu Hidayatul Islam'}</h1>
                  <p className="text-sm font-bold text-emerald-800 uppercase tracking-widest">Panitia Pendaftaran Santri Baru {appSettings?.tahunAjaran || '2026/2027'}</p>
                  <p className="text-[10px] text-slate-500 mt-1">{appSettings?.pondokAddress || 'Jl. Wahyu Hidayat No. 01, Pasuruan, Jawa Timur'}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-1">Kontak Orang Tua</span>
                    <div className="text-sm font-semibold text-slate-700 flex items-center gap-2">
                      <Phone size={14} className="text-emerald-600" />
                      {santri.nomorHpOrangTua}
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-12 pt-6 border-t border-slate-100 flex items-end justify-between">
                ...
                <div className="text-center">
                  <div className="w-24 h-24 bg-slate-50 border border-slate-100 rounded-lg flex items-center justify-center text-[10px] text-slate-300 font-mono mb-2">
                    QR CODE
                  </div>
        ...
        <div className="p-4 bg-slate-50 border-t border-slate-100 flex justify-end no-print">
          <button
            onClick={onClose}
            className="px-6 py-2 bg-slate-200 hover:bg-slate-300 text-slate-700 rounded-xl font-bold text-sm transition-all cursor-pointer"
          >
            Tutup Kartu
          </button>
        </div>
